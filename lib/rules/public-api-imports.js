...
module.exports = {
  meta: {
    ...
    fixable: 'code',
    messages: {
      [PUBLIC_ERROR]: 'Absolute import is only allowed from public api.',
      [TESTING_PUBLIC_ERROR]: 'Test data must be imported from public api.',
    },
    schema: [
      {
        type: 'object',
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        ...
        const isImportsNotFromPublicApi = segments.length > 2;
        const isTestingPublicApi = segments[2] === 'testing' && segments.length < 4;

        if (isImportsNotFromPublicApi && !isTestingPublicApi) {
          context.report({
            node,
